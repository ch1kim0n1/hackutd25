...
export function calculatePortfolioValue(positions: Position[]): number {
  return positions.reduce(
    (total, pos) => total + parseFloat(pos.market_value),
    0,
  );
}
...
export function calculateAveragePrice(orders: Order[]): number {
  const filled = orders.filter(
    (o) => o.filled_qty && parseFloat(o.filled_qty) > 0,
  );
...
export function calculateEMA(bars: Bar[], period: number): number[] {
  ...
  const firstSMA =
    bars.slice(0, period).reduce((acc, bar) => acc + bar.c, 0) / period;
  ...
  for (let i = period; i < bars.length; i++) {
    const value =
      (bars[i].c - ema[ema.length - 1]) * multiplier + ema[ema.length - 1];
 ...
 */
export function validateQuantity(qty: number): {
  valid: boolean;
  error?: string;
} {
 ...
 */
export function sortPositionsByProfitLoss(
  positions: Position[],
  descending: boolean = true,
): Position[] {
  return [...positions].sort((a, b) => {
