...
export async function testAlpacaServices() {
  ...
  try {
    ...
    console.log(
      "   ✓ Time until change:",
      marketStatus.timeUntilChangeFormatted,
    );
    ...
    if (positions.length > 0) {
      positions.forEach((pos) => {
        console.log(
          `     - ${pos.symbol}: ${pos.qty} shares, P/L: $${pos.unrealized_pl}`,
        );
      });
    }
    ...
    if (bars.length > 0) {
      ...
      console.log(
        `     Latest: Open: $${latest.o}, Close: $${latest.c}, Volume: ${latest.v}`,
      );
    }
...
export async function testMarketData(symbol: string = "AAPL") {
  ...
  try {
    ...
    console.log(
      "Today Change: $" + change.change.toFixed(2),
      `(${change.changePercent.toFixed(2)}%)`,
    );
 ...
 */
export async function testTradingValidation(
  symbol: string = "AAPL",
  qty: number = 1,
) {
  ...
  try {
    ...
    // Check buying power
    const price = await alpaca.marketData.getCurrentPrice(symbol);
    const buyingPower = await alpaca.account.getBuyingPowerForSymbol(
      symbol,
      price,
    );
...
export function AlpacaTest() {
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "monospace" }}>
      ...
      <p>
        Open the browser console and click the button below to test all
        services.
      </p>
      <button
        style={{
      ...
      <div style={{ marginTop: "20px" }}>
        ...
        <button
          style={{ margin: "5px", padding: "8px 15px" }}
          onClick={() => testMarketData("AAPL")}
        >
          Test Market Data
        </button>
        <button
          style={{ margin: "5px", padding: "8px 15px" }}
          onClick={() => testTradingValidation("AAPL")}
        >
